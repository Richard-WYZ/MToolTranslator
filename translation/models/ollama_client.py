import requests
import time
from translation.config import ollama_host, system_prompts, think_setting
...
def list_models():
    ...
    url = f"{host}/api/tags"
    try:
        resp = transport_request("ollama", "get", url, timeout=(10, 60))
...
def translate_once(model, text, system_prompt="", terminology=None, timeout=60, options=None, think=None, response_format=None):
    ...
    try:
        ...
        try:
            resp = transport_request("ollama", "post", url, json=payload, timeout=(10, timeout))
        finally:
...
def translate(model, text, system_prompt="", terminology=None, timeout=60, options=None, think=None, response_format=None):
    ...
    for attempt in range(3):
        try:
            return translate_once(model, text, system_prompt=system_prompt, terminology=terminology, timeout=timeout, options=options, think=think, response_format=response_format)
        except requests.exceptions.ConnectionError as e:
            last_error = ConnectionError(f"无法连接到 Ollama ({_host()}): {e}")
        except requests.exceptions.Timeout as e:
            last_error = TimeoutError(
                f"请求超时 (模型={model}, 尝试={attempt + 1}/3): {e}"
            )
        except requests.exceptions.HTTPError as e:
            status = getattr(e.response, "status_code", None)
            if status == 404:
                last_error = RuntimeError(f"模型不存在: {model}")
                break
            last_error = RuntimeError(f"HTTP 错误 (模型={model}): {e}")
        except Exception as e:
            last_error = RuntimeError(f"翻译失败 (模型={model}): {e}")

        # Keep the legacy same-model transport retry for direct callers.
        if attempt < 2:
...
__all__ = ["connection_scope", "get_system_prompts", "list_models", "translate", "translate_once"]
